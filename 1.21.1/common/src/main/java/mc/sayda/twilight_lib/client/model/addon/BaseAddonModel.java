...
public abstract class BaseAddonModel<T extends Entity> extends EntityModel<T> implements IAddonModel {
    ...
    public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
            float headPitch) {
        // Base animation is handled by syncing with player model parts in
        // PlayerAddonLayer
        // This runs AFTER the sync, so player animations take priority
        // Override this method in your addon model to add custom idle animations (e.g.,
        // wagging tail, flapping wings)
    }
    ...
    private void initializeRenderParts() {
        renderParts.clear();
        if (head != null)
            renderParts.add(head);
        if (body != null)
            renderParts.add(body);
        if (rightArm != null)
            renderParts.add(rightArm);
        if (leftArm != null)
            renderParts.add(leftArm);
        if (rightLeg != null)
            renderParts.add(rightLeg);
        if (leftLeg != null)
            renderParts.add(leftLeg);
    ...
    public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay,
        ...
        for (ModelPart part : renderParts) {
            // LOGGER.info("Oooooh! Pretty! Rendering part for model: {}",
            // this.getClass().getSimpleName());
            // Pass white color since tinting is handled by the VertexConsumer
            part.render(poseStack, vertexConsumer, packedLight, packedOverlay, 0xFFFFFFFF);
        }
        if (renderParts.isEmpty()) {
            LOGGER.warn("Is this the best physical representation you can manifest? No parts to render for model: {}",
                    this.getClass().getSimpleName());
        }
    }
