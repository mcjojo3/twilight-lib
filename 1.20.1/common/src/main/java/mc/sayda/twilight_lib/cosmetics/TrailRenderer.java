    ...
    private static void renderSingleTrail(AbstractClientPlayer player, TrailType trailType) {
        ...
        switch (spawnMode) {
            case MOVEMENT:
                ...
                if (horizontalSpeed < TwilightConfig.TRAIL_MIN_SPEED.get())
                    return;
                break;
        ...
        double offsetY = TwilightConfig.TRAIL_FEET_OFFSET_Y.get(); // Start at feet level (works for all entity heights)
        ...
        if (spawnMode == TrailSpawnMode.FOOTPRINT) {
            ...
            double footOffset = TwilightConfig.TRAIL_FOOTPRINT_OFFSET_LATERAL.get(); // Distance from center line
            ...
            for (int i = 0; i < trailType.getParticleCount(); i++) {
                player.level().addParticle(
                        particleType,
                        footX,
                        pos.y + TwilightConfig.TRAIL_FOOTPRINT_OFFSET_Y.get(), // Just barely above ground
                        footZ,
            ...
            for (int i = 0; i < trailType.getParticleCount(); i++) {
                double offsetX = (RANDOM.nextDouble() - 0.5) * TwilightConfig.TRAIL_SPREAD_HORIZONTAL.get();
                double offsetZ = (RANDOM.nextDouble() - 0.5) * TwilightConfig.TRAIL_SPREAD_HORIZONTAL.get();
                double randomY = RANDOM.nextDouble() * TwilightConfig.TRAIL_SPREAD_VERTICAL.get();
