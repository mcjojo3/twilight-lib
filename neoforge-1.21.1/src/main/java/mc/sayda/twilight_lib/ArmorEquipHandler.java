...
public class ArmorEquipHandler {
    ...
    public static void onEquipmentChange(LivingEquipmentChangeEvent event) {
        ...
        double allowAttribute = switch (armorItem.getType()) {
            case HELMET -> player.getAttributeValue(ModAttributes.ALLOW_HELMET);
            case CHESTPLATE -> player.getAttributeValue(ModAttributes.ALLOW_CHESTPLATE);
            case LEGGINGS -> player.getAttributeValue(ModAttributes.ALLOW_LEGGINGS);
            case BOOTS -> player.getAttributeValue(ModAttributes.ALLOW_BOOTS);
            default -> 1.0;
        };
