...
public class ModRequirement {
    ...
    public static boolean shouldLoad(Set<String> modTags) {
        ...
        // We check this FIRST to allow developers/users to override mod requirements
        try {
            ...
            // Config not loaded yet - this is expected during very early initialization
            // (e.g., Enum static blocks)
            // We'll proceed with normal mod detection, but this is why enums might miss
            // items if accessed too early
            if (modTags != null && !modTags.isEmpty()) {
                LOGGER.debug(
                        "Yeah? Well... Config not ready for shouldLoad check (required: {}). Defaulting to mod detection.",
                        String.join(", ", modTags));
            }
        } catch (Exception e) {
            LOGGER.error("How did I?! Uuuughh! Unexpected error checking FORCE_LOAD_ALL_ADDONS", e);
        }
