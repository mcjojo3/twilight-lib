...
public class AmbientEffectHandler {
    ...
    private static void renderParticleCircle(AbstractClientPlayer player, Level level,
        ...
        for (int i = 0; i < cachedParticlesPerTick; i++) {
            ...
            double offsetX = Math.cos(angle) * mc.sayda.twilight_lib.config.TwilightConfig.AMBIENT_CIRCLE_RADIUS.get();
            double offsetZ = Math.sin(angle) * mc.sayda.twilight_lib.config.TwilightConfig.AMBIENT_CIRCLE_RADIUS.get();

            double x = playerPos.x + offsetX;
            double y = playerPos.y + mc.sayda.twilight_lib.config.TwilightConfig.AMBIENT_HEIGHT_OFFSET.get();
    ...
    private static void renderFlameCircle(AbstractClientPlayer player, Level level) {
        renderParticleCircle(player, level,
                ParticleTypes.FLAME, mc.sayda.twilight_lib.config.TwilightConfig.AMBIENT_FLAME_VELOCITY.get(),
                ParticleTypes.SMALL_FLAME,
                mc.sayda.twilight_lib.config.TwilightConfig.AMBIENT_SMALL_FLAME_VELOCITY.get(),
                mc.sayda.twilight_lib.config.TwilightConfig.AMBIENT_SMALL_FLAME_CHANCE.get());
    }
    ...
    private static void renderFrostCircle(AbstractClientPlayer player, Level level) {
        renderParticleCircle(player, level,
                ParticleTypes.SNOWFLAKE, mc.sayda.twilight_lib.config.TwilightConfig.AMBIENT_SNOW_VELOCITY.get(),
                ParticleTypes.WHITE_ASH, mc.sayda.twilight_lib.config.TwilightConfig.AMBIENT_ASH_VELOCITY.get(),
                mc.sayda.twilight_lib.config.TwilightConfig.AMBIENT_ASH_CHANCE.get());
    }
}
