...
public class PlayerAddonLayer extends RenderLayer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, AbstractClientPlayer player,
        ...
        boolean forceAllTranslucent = addons.getActiveAddons().stream()
                .map(id -> {
                    if (id == null)
                        return new ResourceLocation("twilight_lib", "unknown");
                    ResourceLocation resLoc = ResourceLocation.tryParse(id);
                    return resLoc != null ? resLoc
                            : new ResourceLocation("twilight_lib", id.toLowerCase());
                })
            ...
            addonOptional.ifPresent(addon -> {
                ...
                if (addonModel instanceof ChestModel<?>) {
                    ...
                    if (!chestArmor.isEmpty()) {
                        ...
                        if (chestArmor.getItem() instanceof net.minecraft.world.item.ArmorItem armorItem) {
                            ...
                            ResourceLocation armorTexture = new ResourceLocation(domain, "textures/models/armor/" + materialName + "_layer_1.png");
                ...
                try {
                    ...
                    mc.sayda.twilight_lib.api.addon.IAddon addonInterface = (mc.sayda.twilight_lib.api.addon.IAddon) addon;
                    var textureToUse = addonInterface.usePlayerSkin() ? player.getSkinTextureLocation()
                            : addonInterface.getTexture();

                    // Check if THIS addon is the one forcing translucency
                    boolean thisAddonForcesTranslucency = addonInterface.forceAllTranslucent();

                    // Translucency rules (spotlight effect):
                    // 1. If THIS addon is naturally translucent ↁEmake it translucent
                    // 2. If THIS addon is forcing others translucent ↁEkeep THIS opaque (spotlight:
                    // forces others but stays solid)
                    // 3. If ANOTHER addon is forcing translucency ↁEmake THIS translucent (follow
                    // the force)
