 ...
 *
 * <p><b>Purpose:</b> Provides granular control over FOV changes from Pehkui and other mods
 * that modify player size/scale. This is essential for gameplay balance when using size-changing
 * cosmetics or morphs.
 *
 * <p><b>How FOV Scaling Works:</b>
 * <ul>
 *   <li><b>fovModifier = 1.0 (default)</b>: Allow all vanilla FOV changes (passthrough mode)</li>
 *   <li><b>fovModifier = 0.0</b>: Force FOV to 1.0, completely disabling all external FOV changes</li>
 *   <li><b>0.0 &lt; fovModifier &lt; 1.0</b>: Scale FOV changes by this factor (partial suppression)</li>
 * </ul>
 *
 * <p><b>Scaling Algorithm (partial suppression mode):</b>
 * <ol>
 ...
 * </ol>
 *
 * <p><b>Why this asymmetric behavior?</b>
 * <ul>
 *   <li><b>Away from normal</b>: We want to suppress FOV zoom/unzoom caused by scaling</li>
 *   <li><b>Towards normal</b>: We want FOV to return to normal quickly when scaling ends</li>
 *   <li>This creates smooth FOV transitions that feel natural to players</li>
 * </ul>
 *
 * <p><b>Example:</b> Player morphs into a small mob (foxling) with Pehkui scaling:
 * <ul>
 *   <li>Pehkui tries to zoom out FOV from 1.0 → 0.7 (makes small player look bigger on screen)</li>
 *   <li>With fovModifier=0.5, we scale the delta: {@code 1.0 + (0.7-1.0)*0.5 = 0.85}</li>
 *   <li>Result: FOV zooms out less (0.85 instead of 0.7), reducing motion sickness</li>
 *   <li>When morph ends, FOV returns to 1.0 at full speed (unscaled) for quick recovery</li>
 * </ul>
 *
 * <p><b>Thread Safety:</b> lastFovModifier is volatile because it's accessed from render thread.
 *
...
public class FOVHandler {
    ...
    public static synchronized void onFOVModifier(ComputeFovModifierEvent event) {
        ...
        var attributeInstance = player.getAttribute(ModAttributes.FOV_MODIFIER);
        ...
        if (fovModifier >= 1.0) {
            ...
            if (LOGGER.isDebugEnabled() && Math.abs(lastFovModifier - 1.0F) > FOV_DELTA_EPSILON) {
                LOGGER.debug("Well, this is a pretty chill reality. FOV modifier enabled ({}), allowing vanilla FOV changes: {}",
        ...
        // This is key for asymmetric scaling: suppress changes away, allow changes towards normal
        boolean movingTowardsNormal = distanceFromNormal < lastDistanceFromNormal;

        // Case 2: fovModifier <= 0.0 (force normal FOV - completely disable all FOV changes)
        if (fovModifier <= 0.0) {
        ...
        // Case 3: 0.0 < fovModifier < 1.0 (partial suppression - scale FOV changes)
            ...
            // Only scale when moving AWAY from normal (asymmetric scaling for smooth experience)
            if (!movingTowardsNormal && Math.abs(fovDelta) > FOV_DELTA_EPSILON) {
                ...
                // Moving towards normal or negligible change - allow full delta (quick recovery)
